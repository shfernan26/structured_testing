...
def getLaunchfiles(data):
    ...
    for lf in lf_data:
        ...
        if len(lf_path) > 1:
            raise Exception(f"{f} found in multiple place, unsure what to pick {lf_path}")
...
def getBagFiles(data):
    ...
    for b in bag_files:
        if not b.endswith(".bag"):
            raise Exception(f"{f} is not a bag file unable to use")
...
def getObservers(data):
    ...
    for obs in observer_dicts:
        if "observerClass" not in obs:
            raise Exception("Defined Observer without class ensure observerClass present")
        ...
        try:
            obsClass = locate(f"base_observer.{obsClassName}")
            o = obsClass(**obs)
            observers.append(o)
...
if __name__ == '__main__':
    data = getYAMLData("/home/parallels/Development/UWAFT/src/structured_testing/test_files/example.sim")
    # o = data[2][1]
    # print(o)
